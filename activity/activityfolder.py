...
def make_dataset(dir, class_to_idx):
    ...
    #print("dir: ", dir)
    #print("target: ", sorted(os.listdir(dir)))
...
class ActivityImageFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq = [self.loader(path) for path in paths]

        if self.transform is not None:
            new_seq = [self.transform(s) for s in seq]
        if self.target_transform is not None:
            target = self.target_transform(target)

        return torch.stack(new_seq, dim=0), target
